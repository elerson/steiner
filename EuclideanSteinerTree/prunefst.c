...
static boolType		passes_upper_bound_tests (struct pinfo *, struct bsd *, int, int,
...
static boolType		prune_this_fst (struct cinfo *, struct pinfo *, int);
...
int			fstmaxind;
int			isterms;
boolType			all_spanned;
boolType			shorter_pair_found;
struct bbinfo *		bbip;
struct cinfo		lcinfo;
struct edge *		bsdmst;
struct edge *		ep;
...
dist_t			pairl;
	...
	lcinfo.tflag			= NEWA (nverts, boolType);
