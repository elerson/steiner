 ...
 * Local Routines
 */
boolType verificaFullSet2(struct einfo * eip,struct full_set *set);
static void		add_zero_length_fsts (struct einfo *, int, int **);
static void		build_fst_list (struct einfo *);
static boolType		build_efst_graph (struct einfo *,
					  struct point *,
					  struct eqp_t *,
					  struct point **,
					  struct edge **,
					  ...
					  struct point *,
					  int *);
static	boolType   obstacle_segments_intersection (
                                            struct einfo *,
                                            struct point *,
                                            struct point *);
boolType insideObstacle(
                                            struct einfo * eip,  /* IN - global EFST info */
                                            struct point *		po1	/* IN - first endpoint of first segment */
);

boolType verificaFullSet(struct einfo * eip,struct pset * set);
...
static struct full_set ** put_trees_in_array (struct einfo * eip,struct full_set *, int *);
...
static char *		me;
static int		MaxFSTSize = 4;
static int		output_version = 0;
static boolType		Use_Greedy_Heuristic = FALSE;
static boolType		Print_Detailed_Timings = FALSE;
...
struct cinfo* buildSteinerTrees (char *filename)
{
int			i;
int			j;
int			k;
...
int** bsdmat;
	...
	FILE *file = fopen("outGerador","r");
	fpsave = set_floating_point_double_precision ();

	setbuf (stdout, NULL);

	//decode_params (0, NULL);

	
	//pts = get_points (file, &scale);
 	get_efstInFile (file, &scale,&pts,&matrix,&bsdmat);
	...
	cinfo = malloc(sizeof(struct cinfo));
	memset (cinfo, 0, sizeof (cinfo));

	cinfo->num_edges			= einfo.ntrees;
	cinfo->num_verts			= einfo.pts -> n;
        cinfo->intialPolygons            = einfo.pts->polygonInit;
        cinfo->finalPolygons             = einfo.pts->polygonEnd;
	...
	int *remapaux = (int*) malloc(sizeof(int)* einfo.pts -> n);
	int *remap = (int*) malloc(sizeof(int)* einfo.pts -> n);
	for (i = 0; i < einfo.pts -> n; i++)
	{
		remap[i] = 0;
		remapaux[i] = 0;
	}
	...
	cinfo->num_verts = cinfo->num_verts - mmp;
	for (i = 0; i < einfo.ntrees; i++) {
		cinfo->edge [i] = ip1;
		terms = cinfo->full_trees [i] -> terminals;
		k = terms -> n;
		for (j = 0; j < k; j++) {
			*ip1++ = terms -> a [j].pnum - remap[terms -> a [j].pnum] ;
		}
	}
	cinfo->edge [i] = ip1;

	for (i = 0; i < cinfo->num_verts; i++)
	       if(i < cinfo->intialPolygons)
			cinfo -> tflag [i] = TRUE; 
	       else
			cinfo -> tflag [i] = FALSE; 	        
...
)
{
	if (NOT Use_Greedy_Heuristic) {
		return 3000000;//smith_lee_liebman (pts, bsdp);
	}
	else {
		return 3000000;//greedy_heuristic	 (pts, bsdp);
	}
}

/*
...
boolType flag	     /* IN - value of flag */
)
{
	eterm_t *p    = eqp -> Z;
...
)
{
	dist_t rbsd, lbsd;
	if (eqpi -> R EQ NULL) {
		if (eqpj -> R EQ NULL) return eip->bsdmatrix[eqpi -> E.pnum][eqpj -> E.pnum];
...
boolType   append	     /* IN - append to list? */
)
{
	eterm_t *p = eqpk -> Z;
...
)
{
	...
	if (eqpi -> R) {
		...
		for (r = 0; r < eip -> pts -> polygonInit; r++) {
...
)
{
	...
	for (t = 0; t < eip -> pts -> n; t++) {
		...
		//if( eip->pts->a[r].used != 1) continue;

		//if(eqpk->MDIST > 0)
		//	if(EDIST(&(eqpt -> E), &(eqpk->E)) > eqpk->MDIST) continue;
		...
		//if(obstacle_segments_intersection (eip,&(eqpk->E),&(eip->eqp[t].E)) == TRUE) continue;
		...
		if (sqr_dist(&(eqpk -> DC), &(eqpt -> E)) > eqpk -> DR2) {
			...
			if(obstacle_segments_intersection (eip,&(SP),&(eqpt -> E)) == TRUE) continue;
			...
			test_and_save_fst(eip, eqpt, eqpk,TRUE);
                           // return FALSE;
		}
	}
...
struct elist		*rp;
	...
	//eip -> max_mst_edge = eip -> max_mst_edge*3; //
       // eip -> max_mst_edge *=100;
/*
 *
        printf("%f ",eip -> max_mst_edge);
*/
	eip -> mst_length = mst_len;
        eip -> max_mst_edge = 3000000.0;
	if (Print_Detailed_Timings) {
		convert_delta_cpu_time (buf1);
	...
	*/
	...
	eip -> term_check	= NEWA (n, boolType);
			...
			while (*eqpp) {
				...
				for (iter = 1; iter <= 3; iter++) {
					...
					eqpk -> MDIST = -1 ;
                                       ...
                                       // printf("1)%f %f \n",eqpi -> E.x,eqpi -> E.y);
				//	printf("2)%f %f \n",eqpk -> RP.x,eqpk -> RP.y);
                                        //obstacle_intersection(eip, eqpi, eqpj, eqpk);
					...
					if (NOT lune2(eip, eqpi, eqpj, eqpk))			 continue;
					...
					if (eqpk -> S > 2) {
						...
						upper_bound = 30000000;// upper_bound_heuristic(eip -> termlist, eip -> bsd);
...
#endif
              //  boolType test = TRUE;
                if(eip-> matrix[i][j] == 0) continue;
                //printf("(%d %d) ",i,j);
		eip -> termlist -> a[0] = pts -> a[ i ];
		eip -> termlist -> a[1] = pts -> a[ j ];
...
found_efst:
	...
	if (size <= 2) {
		...
		edges -> len  = length;
                //if(test == TRUE)
               // if( obstacle_segments_intersection ( eip,&(eip -> termlist -> a [0]),&(eip -> termlist -> a [1]))
                 // == TRUE) return -1;
		...
		ep -> len   = EDIST(&(eqpt -> E), &nsp);
         ...
         //       printf("%f %f --- %f %f \n",sp->x,sp->y,eqpt->E.x,eqpt->E.y);

              //  if(obstacle_segments_intersection ( eip, &(eqpt -> E),sp)
                //  == TRUE )
	          //      {
		//		spidx = spidxaux;
		  //  		termidx = termidxaux;
		...
		if(build_efst_graph (eip, &nsp, eqpk -> R, &sp, &ep, &spidx, tp, &termidx) == FALSE)
		{
		    sp = spaux;
		    ep = epaux;
	            spidx = spidxaux;
		...
		if(build_efst_graph (eip, &nsp, eqpk -> L, &sp, &ep, &spidx, tp, &termidx) == FALSE)
		{
		   sp = spaux;
		   ep = epaux;
		   spidx = spidxaux;
		...
		/*if(verificaFullSet(eip,new_steiners))
		{
			sp = spaux;
		   	ep = epaux;
		   	spidx = spidxaux;
	...
	if(verificaFullSet2(eip,fsp) == TRUE )
	{
		//sp = spaux;
		//ep = epaux;
		return - 1;
...
struct point		nnsp;
	...
	if (eqpk -> R EQ NULL) {
		...
		(*ep) -> len = EDIST (nsp, &(eqpk -> E));
               ...
               // if(
                 //      obstacle_segments_intersection ( eip,nsp,&(eip -> termlist -> a [idx]))
                 // == TRUE) return FALSE;
                
		if(idx > eip -> pts -> a [eip->pts->polygonEnd].pnum) 
                	if(insideObstacle(eip,&(eip -> termlist -> a [idx])) == TRUE ) return FALSE;
		
		if(nsp->pnum > eip -> pts -> a [eip->pts->polygonEnd].pnum) 
                	if(insideObstacle(eip, nsp) == TRUE ) return FALSE;
		...
		(*ep) -> len  = EDIST (nsp, &nnsp);
               ...
               // if( obstacle_segments_intersection ( eip,*sp,nsp)
                 // == TRUE) return FALSE;
		
		if(idx > eip->pts->a[eip->pts->polygonEnd].pnum) 
                	if(insideObstacle(eip,&(eip -> termlist -> a [idx])) == TRUE ) return FALSE;

		if(nnsp.pnum > eip->pts->a[eip->pts->polygonEnd].pnum) 
                	if(insideObstacle(eip,&nnsp) == TRUE ) return FALSE;
		

                (*sp)++;
		(*ep)++;

		if(build_efst_graph (eip, &nnsp, eqpk -> R, sp, ep, spidx, tp, termidx) == FALSE)
		{	
		  (*sp)--;
		  (*ep)--;
                  return FALSE;
		}
		if(build_efst_graph (eip, &nnsp, eqpk -> L, sp, ep, spidx, tp, termidx) == FALSE)
		{
                  (*sp)--;
		  (*ep)--;
		  return FALSE;
...
struct eqp_t * eqpk  /* IN - new eq-point */
     ...
     int inicial,final,i;
     struct point CLP, CRP;
     struct eqp_t *pii,*p1,*p2;
     boolType left,right,basedir,testdir1,testdir2,inside1,inside2;
	if (eqpi -> R) {
            inicial =  eip->pts->polygonInit;
            final   =  eip->pts->polygonEnd;
            //take obstacles
            ...
            for(i=inicial;i<=final;i++)
            {
                ...
                right = right_turn(&(p1->E), &(eqpk -> LP) , &(p2->E));
                left = left_turn(&(p1->E), &(eqpk -> RP) , &(p2->E));
                //probably inside arc
                if(right == left )
                {
        //            printf("%f %f|%f %f|%f %f (%d)\n",p1->E.x,p1->E.y,eqpk -> LP.x,eqpk -> LP.y,p2->E.x,p2->E.y,right);
//printf("%f %f|%f %f|%f %f (%d)\n",p1->E.x,p1->E.y,eqpk -> RP.x,eqpk -> RP.y,p2->E.x,p2->E.y,left);


                    inside1 = 0;
                    inside2 = 0;
                    basedir = left_turn(&(eqpk -> LP), &(eqpk -> E) , &(eqpk -> RP));

                    //direita

                    //first point
                    if(sqr_dist(&(p1->E),&(eqpk -> DC)) < eqpk -> DR)
                    {
                        testdir1 = left_turn(&(eqpi -> RP), &(p1->E) , &(eqpk -> RP));
                        if(basedir != testdir1)// do projection
                        {
                            //project_point(&(eqpi -> RP),  &(eqpk -> DC),&(p1->E), &(eqpk -> RP));
                            inside1 = 1;
                        }

                    }
                    //second point
                    if(sqr_dist(&(p2->E),&(eqpk -> DC)) < eqpk -> DR2)
                    {
                        testdir2 = left_turn(&(eqpi -> RP), &(p2->E) , &(eqpk -> RP));
                        if(basedir != testdir2)// do projection
                        {
                            //project_point(&(eqpi -> RP),  &(eqpk -> DC),&(p2->E), &(eqpk -> RP));
                            inside2 = 1;
                        }
                    ...
                    if(inside2 != inside1)
                    {
                        if(testdir1)
                        {
                            project_point(&(p1->E),  &(eqpk -> DC),&(p2->E), &(eqpk -> RP));
                        }
                        else
                        {
                            project_point(&(p2->E),  &(eqpk -> DC),&(p1->E), &(eqpk -> RP));
                        }

                    }else
                        if(inside2 && inside1)
                        {
                            if(basedir == left_turn(&(eqpk -> LP), &(p1->E) , &(p2->E)))
                            {
                                project_point(&(eqpi -> RP),  &(eqpk -> DC),&(p2->E), &(eqpk -> RP));
                            }
                            else
                    ...
                    //first point
                    if(sqr_dist(&(p1->E),&(eqpk -> DC)) < eqpk -> DR2)
                    {

                        testdir1 = left_turn(&(eqpi -> LP), &(p1->E) , &(eqpk -> LP));
                        if(basedir != testdir1)// do projection
                        {
                            //project_point(&(eqpi -> RP),  &(eqpk -> DC),&(p1->E), &(eqpk -> RP));
                            inside1 = 1;
                    ...
                    if(sqr_dist(&(p2->E),&(eqpk -> DC)) < eqpk -> DR2)
                    {
                        testdir2 = left_turn(&(eqpi -> LP), &(p2->E) , &(eqpk -> LP));
                        if(basedir != testdir2)// do projection
                        {
                            //project_point(&(eqpi -> RP),  &(eqpk -> DC),&(p2->E), &(eqpk -> RP));
                            inside2 = 1;
                        ...
                        if(inside2 && inside1)
                        {
                            if(basedir == left_turn(&(eqpk -> LP), &(p1->E) , &(p2->E)))
                            {
                                project_point(&(eqpi -> LP),  &(eqpk -> DC),&(p2->E), &(eqpk -> LP));

                            }
                            else
                            {
                                project_point(&(eqpi -> LP),  &(eqpk -> DC),&(p1->E), &(eqpk -> LP));
     ...
     if (eqpj -> R) {
            ...
            for(i=inicial;i<=final;i++)
            {
                ...
                if(right == left )
                {
                    ...
                    if(sqr_dist(&(p1->E),&(eqpk -> DC)) < eqpk -> DR2)
                    {
                        testdir1 = left_turn(&(eqpj -> RP), &(p1->E) , &(eqpk -> RP));
                        if(basedir != testdir1)// do projection
                        {
                            //project_point(&(eqpi -> RP),  &(eqpk -> DC),&(p1->E), &(eqpk -> RP));
                            inside1 = 1;
                    ...
                    if(sqr_dist(&(p2->E),&(eqpk -> DC)) < eqpk -> DR2)
                    {
                        testdir2 = left_turn(&(eqpj -> RP), &(p2->E) , &(eqpk -> RP));
                        if(basedir != testdir2)// do projection
                        {
                            //project_point(&(eqpi -> RP),  &(eqpk -> DC),&(p2->E), &(eqpk -> RP));
                            inside2 = 1;
                    ...
                    if(inside2 != inside1)
                    {
                        ...
                        if(inside2 && inside1)
                        {
                            if(basedir == left_turn(&(eqpk -> LP), &(p1->E) , &(p2->E)))
                            {
                                project_point(&(eqpj -> RP),  &(eqpk -> DC),&(p2->E), &(eqpk -> RP));
                            }
                            else
                            {
                                project_point(&(eqpj -> RP),  &(eqpk -> DC),&(p1->E), &(eqpk -> RP));
                            }
                        }
                    ...
                    if(sqr_dist(&(p1->E),&(eqpk -> DC)) < eqpk -> DR2)
                    {
                        ...
                        testdir1 = left_turn(&(eqpj -> LP), &(p1->E) , &(eqpk -> LP));
                        if(basedir != testdir1)// do projection
                        {
                            //project_point(&(eqpi -> RP),  &(eqpk -> DC),&(p1->E), &(eqpk -> RP));
                            inside1 = 1;
                    ...
                    if(sqr_dist(&(p2->E),&(eqpk -> DC)) < eqpk -> DR2)
                    {
                        testdir2 = left_turn(&(eqpj -> LP), &(p2->E) , &(eqpk -> LP));
                        if(basedir != testdir2)// do projection
                        {
                            //project_point(&(eqpi -> RP),  &(eqpk -> DC),&(p2->E), &(eqpk -> RP));
                            inside2 = 1;
                        ...
                        if(inside2 && inside1)
                        {
                            if(basedir == left_turn(&(eqpk -> LP), &(p1->E) , &(p2->E)))
                            {
                                project_point(&(eqpj -> LP),  &(eqpk -> DC),&(p2->E), &(eqpk -> LP));

                            }
                            else
                            {
                                project_point(&(eqpj -> LP),  &(eqpk -> DC),&(p1->E), &(eqpk -> LP));
...
struct eqp_t * eqpk  /* IN - new eq-point */
    ...
    int obstaclePnumMin,obstaclePnumMax;
    int obstNum,polyNum,i;
    struct eqp_t *pii;
    double dx,dy,ndx,ndy;
    struct point p;
    obstaclePnumMin = eip->pts->polygonInit;
    obstaclePnumMax = eip->pts->polygonEnd;
    if(eqpi->E.pnum > obstaclePnumMin && eqpi->E.pnum < obstaclePnumMax)
    {
        obstNum = eqpi->E.pnum;
        polyNum = eip->eqp[obstNum - 1].E.polygon;
        //anterior
        if(eqpi->E.polygon != polyNum)

            for(i = obstNum; i <= obstaclePnumMax;i++)
            {
                pii = &(eip->eqp[i]);
                if(eip->eqp[i+1].E.polygon != polyNum ) break;
            }

         get_angle_vector (&(eqpk->LP),&(eqpi->E),&(pii->E),&dx,&dy);
         if(!angle_geq_120 (dx,dy))
         {
               ndx = dx/(dx + dy);
               ndy = dy/(dx + dy);
               rotate2 (&(eqpi->E),&(pii->E),(ndy-0.580611184),(ndx-0.814180971),&p);

           project_point(&(eqpi->E),  &(eqpk -> DC),&p, &(eqpk -> LP));
       ...
       //posterior
         if(eqpi->E.polygon != polyNum)
            for(i = obstNum; i >=  obstaclePnumMin;i--)
            {
                pii = &(eip->eqp[i]);
                if(eip->eqp[i-1].E.polygon != polyNum ) break;
            }

          get_angle_vector (&(pii->E),&(eqpk->RP),&(eqpi->E),&dx,&dy);
          if(!angle_geq_120 (dx,dy))
          {

                ndx = dx/(dx + dy);
                ndy = dy/(dx + dy);
                rotate2 ( &(eqpi->E),&(pii->E),0.580611184 - ndy,0.814180971 - ndx,&p);
                project_point(&(eqpi->E),  &(eqpk -> DC),&p, &(eqpk -> RP));
          }
...
static	boolType
obstacle_segments_intersection (
struct einfo * eip,  /* IN - global EFST info */
struct point *		po1,	/* IN - first endpoint of first segment */
struct point *		po2	/* IN - second endpoint of first segment */
)
{
    
     int inicial,final,i;
     
     struct point *pii,*p1,*p2;
        ...
        final   =  eip->pts->polygonEnd;
            ...
            pii = &(eip -> pts->a[inicial]);
            for(i=inicial;i<=final;i++)
            {
                 p1 = &(eip -> pts->a[i]);
                 if(i + 1 < eip->pts->n ){
                     p2 = &(eip -> pts->a[i + 1]);
                     if(p1->polygon != p2->polygon)
                     {
                         p2=pii;
                         pii = &(eip -> pts->a[i + 1]);
                     }
             ...
             if(segment_intersection2(po1,po2,p1,p2,NULL) == TRUE)
                     return TRUE;
            }
           // printf("kkkksadsadsad \n");
            return FALSE;
}
...
int i;
	...
	{
			//printf("((((((((((((((((((((( %f %f\n", set->a[i].x,set->a[i].y);
			if(insideObstacle(eip,&(set->a[i]))) 
			{
				//printf("kkkksadsadsad %f %f\n", set->a[i].x,set->a[i].y);
				return TRUE;
			}
	}
...
struct point p1,p2;
int j;
	for(j =0;j < set->nedges;j++)
        {

               xindice = set->edges[j].p1;
               yindice = set->edges[j].p2;

	       double x1,y1,x2,y2,id;

               if(xindice < set->terminals->n )
               {
                    p1.x = set->terminals->a[xindice].x;
                    p1.y = set->terminals->a[xindice].y;
               }else{
                   p1.x = set->steiners->a[xindice - set->terminals->n].x;
                   p1.y = set->steiners->a[xindice - set->terminals->n].y;
                   // printf("e[%d %d %d %d]",x,y,xindice, info->full_trees[i]->terminals->n);
               }
		

	       if(yindice < set->terminals->n )
               {
                    p2.x = set->terminals->a[yindice].x;
                    p2.y = set->terminals->a[yindice].y;
               }else{
                   p2.x = set->steiners->a[yindice - set->terminals->n].x;
                   p2.y = set->steiners->a[yindice - set->terminals->n].y;
                   // printf("e[%d %d %d %d]",x,y,xindice, info->full_trees[i]->terminals->n);
               }

		if(obstacle_segments_intersection(eip,&p1,&p2) == TRUE) return TRUE;
...
double distaux;
	...
	for(r = 0; r < eip -> pts -> n; r++) {
		if((eip->pts->a[r].polygon != -1 )&&( eqpi->E.polygon != -1) &&( eqpj->E.polygon != -1))
			if((eip->pts->a[r].polygon == eqpi->E.polygon) && (eip->pts->a[r].polygon == eqpj->E.polygon)) continue;

		
		
		if( eip->pts->a[r].used == 1)
		     if(sqr_dist(&(eqpk -> DC), &(eip -> eqp[r].E)) >= eqpk -> DR)
			if(right_turn (&(eqpk->E),&(eqpi->E),&(eip -> eqp[r].E)) != right_turn (&(eqpk->E),&(eqpj->E),&(eip -> eqp[r].E)))
			{
				project_point(&(eqpk -> E), &(eqpk -> DC), &(eip -> eqp[r].E),&NLP );
				if(obstacle_segments_intersection (eip,&(NLP),&(eip->eqp[r].E)) == FALSE)
				{					
					flag = TRUE;
					if(eip->pts->a[r].polygon == -1)
					{
						distaux = EDIST(&(eqpk -> E), &(eip -> eqp[r].E));
						if((eqpk->MDIST < distaux) || (eqpk->MDIST < 0))
						{
							eqpk->MDIST = distaux;				
						}
					}
