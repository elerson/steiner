...
struct constraint *	find_small_subtours (struct comp *,
					     struct constraint *,
					     struct bbinfo *);
boolType			is_equal (bitmap_t *, bitmap_t *, int);
...
coord_t			y;
	...
	pts = (struct pset *) newT (PSET_SIZE (nverts));
...
boolType			do_supersets,	/* IN - do supersets of violations? */
struct comp *		comp,		/* IN - component to enumerate */
struct constraint *	cp,		/* IN - existing constraints */
struct bbinfo *		bbip		/* IN - branch-and-bound info */
)
