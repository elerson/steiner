...
boolType		add_constraint_to_pool (struct cpool *,
					struct rcoef *,
					boolType);
...
void		initialize_constraint_pool (struct cpool *,
					    ...
					    bitmap_t *,
					    struct cinfo *);
boolType		is_violation (struct rcoef *, double *);
...
boolType			Seed_Pool_With_2SECs = TRUE;
...
#ifdef LPSOLVE
boolType			Use_Perturbations = FALSE;
boolType			Use_Scaling = FALSE;
#endif
...
static void		prune_pending_rows (struct bbinfo *, boolType);
...
static boolType		sprint_term (char *, boolType, int, int);
...
boolType			add_to_lp	/* IN - add it to LP tableaux also? */
)
{
int		hval;
int		len;
...
double *		dj;
struct cpool *		pool;
boolType			any_violations;
boolType			can_delete_slack;
int			pool_iteration;
double			slack;
double			prev_z;
...
struct bbinfo *		bbip,		/* IN - branch-and-bound info */
boolType			can_del_slack	/* IN - OK to delete slack rows? */
)
{
int		i;
int		k;
...
#undef GRACE_TIME
	...
	delflags = NEWA (pool -> nrows, boolType);
...
boolType		first,		/* IN - TRUE iff first term. */
...
struct bbinfo *	bbip,		/* IN - branch-and-bound info */
boolType		only_LP		/* IN - display only rows in the LP? */
)
{
int		i;
int		k;
