...
int *			ip2;
	...
	cip -> tflag		= NEWA (nverts, boolType);
...
char			buf1 [128];
	...
	if (geometric) {
		...
		for (i = 0; i < nverts; i++) {
			...
			(void) get_dec_double ();
			(void) get_dec_double ();
                        p1 -> polygon = get_dec_double ();
			p1 -> x		 = get_hex_double ();
			p1 -> y		 = get_hex_double ();
                       // p1 -> polygon	 = get_hex_double ();
			p1 -> pnum	 = i;
		}
	}
...
double		x;
	...
	if (scanf (" %lg", &x) NE 1) {
		fprintf (stderr, "Expected floating point number %d.\n",x);
		exit (1);
	}
