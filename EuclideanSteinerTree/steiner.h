...
typedef	int		bitmap_t;	/* Element of Bit-map vector. */
...
struct point {
	...
	pnum_t		pnum;
        int used;
        int polygon;  // it begongs to witch polygon - -1 for anyone
        int vertice;  //witch vertice on polygon it belongs to
};
...
#define	NEW(type)	((type *) newT (sizeof (type)))
#define	NEWA(n, type)	((type *) newT ((size_t) ((n) * sizeof (type))))


#define	NULL_PSET	((struct pset *) 0)
#define	PSET_SIZE(n)	(offsetof (struct pset, a [n]))
#define NEW_PSET(n)	((struct pset *) newT (PSET_SIZE (n)))
...
struct cinfo {
	...
	boolType*			tflag;		/* TRUE: vertex is terminal, */
	...
	dist_t			mst_length;
        int intialPolygons,finalPolygons;
...
extern boolType		decode_cpu_time_limit (char *, int32u *);
...
//extern void get_efstInFile (FILE *, struct scale_info*,struct pset *,struct bsd *);
