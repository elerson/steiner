...
boolType incomp2(struct cinfo *info,int i,int j);
...
mintree.nmm = 0;
	...
	// incialize used data structures
        mintree.treeUsed = malloc(sizeof(int)*cinfo->num_edges);
        mintree.vertices = malloc(sizeof(int)*cinfo->pts->n);
        mintree.usedPoints = malloc(sizeof(int)*cinfo->pts->n);
        mintree.sets = malloc(sizeof(int)*cinfo->num_edges);
        ...
        mintree.matrix = malloc(sizeof(int*)*cinfo->num_verts);
        for(i=0;i<cinfo->num_verts;i++)
            mintree.matrix[i] = malloc(sizeof(int)*cinfo->num_verts);
        ...
        mintree.matrix2 = malloc(sizeof(int*)*cinfo->num_verts);
        for(i=0;i<cinfo->num_verts;i++)
            mintree.matrix2[i] = malloc(sizeof(int)*cinfo->num_verts);
	...
	if(num > 1)
       ...
       //apply point mask
                ...
                if(flag == 0)
		{
			...
			if(verifiesCyclePruneLast_b(info,newLoopMask,newIncompMaskloop,i+1) == TRUE)				
                   		findBestTree_b(info,newLoopMask,newIncompMaskloop,nB,nC,val,i+1);
		}
